...
// Check the timezone, if it is not set; the default to UTC.
function checkZone(){
...
function addOrSubtractMethod(fn) {
    ...
    return function (num, unit) {
        ...
        if (typeof num !== 'number') {
            throw new Error('duration must be defined: ' + num + ' : ' + (typeof num));
        }
...
moment.fn.addPacificAucklandUTCOffsetToDate = function addPacificAucklandUTCOffsetToDate() {
    var pacificAucklandDate = moment.tz(this, "Pacific/Auckland");
    var offsetInMinutes = pacificAucklandDate.utcOffset();
    var dateWithOffsetAdded = moment(this).add(offsetInMinutes, 'minutes');
    return dateWithOffsetAdded;
}

moment.fn.removePacificAucklandUTCOffsetFromDate = function removePacificAucklandUTCOffsetFromDate() {
    var pacificAucklandDate = moment.tz(new Date(), "Pacific/Auckland");
    var offsetInMinutes = pacificAucklandDate.utcOffset();
    var dateWithOffsetRemoved = moment(this).subtract(offsetInMinutes, 'minutes');
    return dateWithOffsetRemoved;
}

moment.fn.endOfWorkingDay = function() {
    var openinghours = openingTimes(this);
    if (!openinghours) {
        return null;
    }
    else if (!this.isWorkingDay()) {
        return null;
    }
    else {
...
moment.fn.isWorkingTime = function isWorkingTime() {
    ...
    if (!openinghours) {
        ...
        return this.isSameOrAfter(openinghours[0]) && this.isBefore(openinghours[1]);
    }
};
